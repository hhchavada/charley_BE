...
describe('ResultBuilder', () => {
  ...
  const createMockGrantResult = (id: string, state: GrantState): GrantEvaluationResult => ({
    grant: { grantId: id, name: `Grant ${id}` } as any,
    state,
    score: 10,
    matchedRulesCount: 0,
    failedRulesCount: 0,
  ...
  const mockRanking: RankingResult = {
    readyNow: [{
      grantResult: createMockGrantResult('g1', GrantState.ELIGIBLE),
  ...
  it('generates completely structured AssessmentResultDTO', async () => {
    const result = await builder.build(
      mockContext,
      mockMetrics,
      [createMockGrantResult('g1', GrantState.ELIGIBLE)],
      [],
      [],
      [],
      undefined,
