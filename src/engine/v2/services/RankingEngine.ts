import { IRankingEngine } from '../interfaces/dependencies';
import { 
...
export class RankingEngine implements IRankingEngine {
  ...
  public async rank(grants: GrantEvaluationResult[]): Promise<RankingResult> {
    ...
    const readyNow: RankedGrantDTO[] = [];
    const needsInformation: RankedGrantDTO[] = [];
    const prepareNext: RankedGrantDTO[] = [];
    const windowClosed: RankedGrantDTO[] = [];
    ...
    const rawRanked: RankedGrantDTO[] = grants.map(g => {
    ...
    const mergeGroups = new Map<string, RankedGrantDTO[]>();
    const standaloneGrants: RankedGrantDTO[] = [];

    for (const r of rawRanked) {
      const mergeGroup = (r.grantResult.grant as any).mergeGroup || r.grantResult.grant.metadata?.mergeGroup;
      if (mergeGroup) {
        if (!mergeGroups.has(mergeGroup)) mergeGroups.set(mergeGroup, []);
    ...
    const processedGrants: RankedGrantDTO[] = [...standaloneGrants];
    ...
    for (const [groupName, streams] of mergeGroups.entries()) {
      ...
      streams.sort((a, b) => {
        const pA = (a.grantResult.grant as any).streamPriority ?? a.grantResult.grant.metadata?.streamPriority ?? 0;
        const pB = (b.grantResult.grant as any).streamPriority ?? b.grantResult.grant.metadata?.streamPriority ?? 0;
        return pB - pA;
      });

      const recommendedStream = streams[0].grantResult;
      const mergedCard: RankedGrantDTO = {
    ...
    processedGrants.sort((a, b) => {
      const priorityA = (a.grantResult.grant as any).priority || a.grantResult.grant.metadata?.priority || 99;
      const priorityB = (b.grantResult.grant as any).priority || b.grantResult.grant.metadata?.priority || 99;
      if (priorityB !== priorityA) return priorityA - priorityB; // Ascending: 1 is highest priority
    ...
    for (const item of processedGrants) {
      ...
      if (state === GrantState.NOT_ELIGIBLE) {
      ...
      if (state === GrantState.POTENTIALLY_ELIGIBLE) {
   ...
   */
  private calculateScore(g: GrantEvaluationResult): number {
    const meta = g.grant.metadata || {};
    let score = 0;
