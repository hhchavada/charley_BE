import { GrantDTO, RuleDTO, RuleGroupDTO, QuestionDTO } from '../dto';
import { GrantGraph, RuleGroupGraph, RuleGraph, QuestionGraph } from '../interfaces';

export class GraphBuilder {
  static build(
    grants: GrantDTO[],
    ruleGroups: RuleGroupDTO[],
    rules: RuleDTO[],
    questions: QuestionDTO[]
  ): GrantGraph[] {
    const questionMap = new Map<string, QuestionGraph>();
    const ruleMap = new Map<string, RuleGraph>();
    const groupMap = new Map<string, RuleGroupGraph>();
    ...
    ruleGroups.forEach(g => {
      ...
      g.rules.forEach(ruleId => {
        ...
        if (ruleNode) groupNode.rules.push(ruleNode);
      });
    ...
    ruleGroups.forEach(g => {
      ...
      g.nestedGroups.forEach(nestedId => {
        ...
        if (nestedNode) groupNode.nestedGroups.push(nestedNode);
      });
    });

    // 5. Build Grant graphs
    const grantGraphs: GrantGraph[] = [];
