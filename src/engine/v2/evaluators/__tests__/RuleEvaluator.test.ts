...
describe('RuleEvaluator', () => {
  ...
  describe('Missing Data Handling', () => {
    it('returns MISSING if field is undefined', () => {
      ...
      expect(RuleEvaluator.evaluate(rule, {})).toBe("MISSING" as any);
    });

    it('returns MISSING if field is null', () => {
      const rule = createRule('equals', 'abc');
      expect(RuleEvaluator.evaluate(rule, { field: null })).toBe("MISSING" as any);
    });

    it('returns MISSING if field is empty string', () => {
      const rule = createRule('equals', 'abc');
      expect(RuleEvaluator.evaluate(rule, { field: '' })).toBe("MISSING" as any);
    });
  });
  ...
  describe('exists and not_exists Operators', () => {
    it('exists returns PASS if value is present', () => {
      ...
      expect(RuleEvaluator.evaluate(rule, { field: 'value' })).toBe("PASS" as any);
    });

    it('exists returns FAIL if value is missing', () => {
      const rule = createRule('exists', null);
      expect(RuleEvaluator.evaluate(rule, {})).toBe("FAIL" as any);
    });

    it('not_exists returns PASS if value is missing', () => {
      const rule = createRule('not_exists', null);
      expect(RuleEvaluator.evaluate(rule, {})).toBe("PASS" as any);
    });

    it('not_exists returns FAIL if value is present', () => {
      const rule = createRule('not_exists', null);
      expect(RuleEvaluator.evaluate(rule, { field: 'value' })).toBe("FAIL" as any);
    });
  });
  ...
  describe('equals Operator', () => {
    it('passes for matching strings', () => {
      ...
      expect(RuleEvaluator.evaluate(rule, { field: 'hello' })).toBe("PASS" as any);
    });

    it('fails for different strings', () => {
      const rule = createRule('equals', 'hello');
      expect(RuleEvaluator.evaluate(rule, { field: 'world' })).toBe("FAIL" as any);
    });

    it('passes for matching numbers', () => {
      const rule = createRule('equals', 100);
      expect(RuleEvaluator.evaluate(rule, { field: 100 })).toBe("PASS" as any);
    });

    it('passes for matching booleans', () => {
      const rule = createRule('equals', true);
      expect(RuleEvaluator.evaluate(rule, { field: true })).toBe("PASS" as any);
    });

    it('passes for matching dates (Date objects/strings)', () => {
      const rule = createRule('equals', '2023-01-01T00:00:00.000Z');
      expect(RuleEvaluator.evaluate(rule, { field: new Date('2023-01-01T00:00:00.000Z') })).toBe("PASS" as any);
    });

    it('passes for matching arrays', () => {
      const rule = createRule('equals', [1, 2, 3]);
      expect(RuleEvaluator.evaluate(rule, { field: [1, 2, 3] })).toBe("PASS" as any);
    });

    it('fails for arrays with different lengths or items', () => {
      const rule = createRule('equals', [1, 2, 3]);
      expect(RuleEvaluator.evaluate(rule, { field: [1, 2] })).toBe("FAIL" as any);
      expect(RuleEvaluator.evaluate(rule, { field: [1, 3, 2] })).toBe("FAIL" as any);
    });
  });
  ...
  describe('not_equals Operator', () => {
    it('passes for different values', () => {
      ...
      expect(RuleEvaluator.evaluate(rule, { field: 'world' })).toBe("PASS" as any);
    });

    it('fails for identical values', () => {
      const rule = createRule('not_equals', 'hello');
      expect(RuleEvaluator.evaluate(rule, { field: 'hello' })).toBe("FAIL" as any);
    });
  });
  ...
  describe('Numeric Comparisons', () => {
    it('greater_than passes', () => {
      ...
      expect(RuleEvaluator.evaluate(rule, { field: 150 })).toBe("PASS" as any);
    });

    it('greater_than fails', () => {
      const rule = createRule('greater_than', 100);
      expect(RuleEvaluator.evaluate(rule, { field: 50 })).toBe("FAIL" as any);
    });

    it('greater_than fails on equal', () => {
      const rule = createRule('greater_than', 100);
      expect(RuleEvaluator.evaluate(rule, { field: 100 })).toBe("FAIL" as any);
    });

    it('less_than passes', () => {
      const rule = createRule('less_than', 100);
      expect(RuleEvaluator.evaluate(rule, { field: 50 })).toBe("PASS" as any);
    });

    it('less_than fails', () => {
      const rule = createRule('less_than', 100);
      expect(RuleEvaluator.evaluate(rule, { field: 150 })).toBe("FAIL" as any);
    });

    it('returns ERROR if values cannot be parsed to float', () => {
      const rule = createRule('greater_than', 100);
      expect(RuleEvaluator.evaluate(rule, { field: 'not_a_number' })).toBe("ERROR" as any);
    });
  });
  ...
  describe('contains and not_contains', () => {
    it('contains passes if array includes value', () => {
      ...
      expect(RuleEvaluator.evaluate(rule, { field: ['agri', 'tech', 'retail'] })).toBe("PASS" as any);
    });

    it('contains fails if array does not include value', () => {
      const rule = createRule('contains', 'tech');
      expect(RuleEvaluator.evaluate(rule, { field: ['agri', 'retail'] })).toBe("FAIL" as any);
    });

    it('contains passes if string includes substring', () => {
      const rule = createRule('contains', 'hello');
      expect(RuleEvaluator.evaluate(rule, { field: 'say hello world' })).toBe("PASS" as any);
    });

    it('not_contains passes if array excludes value', () => {
      const rule = createRule('not_contains', 'tech');
      expect(RuleEvaluator.evaluate(rule, { field: ['agri', 'retail'] })).toBe("PASS" as any);
    });
    
    it('returns FAIL if contains is used on a number (not array/string)', () => {
      const rule = createRule('contains', 'tech');
      expect(RuleEvaluator.evaluate(rule, { field: 12345 })).toBe("FAIL" as any);
    });
  });
  ...
  describe('regex Operator', () => {
    it('passes on match', () => {
      ...
      expect(RuleEvaluator.evaluate(rule, { field: 'SME-123' })).toBe("PASS" as any);
    });

    it('fails on no match', () => {
      const rule = createRule('regex', '^SME-');
      expect(RuleEvaluator.evaluate(rule, { field: 'CORP-123' })).toBe("FAIL" as any);
    });

    it('fails gracefully on invalid regex pattern provided as expected value', () => {
      const rule = createRule('regex', '['); // Invalid regex syntax
      expect(RuleEvaluator.evaluate(rule, { field: 'anything' })).toBe("FAIL" as any);
    });

    it('fails if field is not a string', () => {
      const rule = createRule('regex', '^1');
      expect(RuleEvaluator.evaluate(rule, { field: 123 })).toBe("FAIL" as any);
    });
  });
  ...
  describe('Nested Paths & Edge Cases', () => {
    it('evaluates nested paths correctly', () => {
      ...
      expect(RuleEvaluator.evaluate(rule, payload)).toBe("PASS" as any);
    });

    it('returns MISSING if nested path does not exist', () => {
      const rule = createRule('equals', 50, 'company.financials.profit');
      const payload = { company: { financials: { revenue: 50 } } };
      expect(RuleEvaluator.evaluate(rule, payload)).toBe("MISSING" as any);
    });
