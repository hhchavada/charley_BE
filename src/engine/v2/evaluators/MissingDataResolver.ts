...
export class MissingDataResolver implements IMissingDataResolver {
  ...
  public async resolve(
    ...
    const diagnostics = {
      ...
      invalidQuestions: [] as string[]
    };

    // 1. Build a lookup map of all Rules -> Questions from the ConfigurationBundle
    const ruleToQuestionMap = new Map<string, { question: QuestionGraph, ruleIds: Set<string> }>();
    const grantUnlockMap = new Map<string, Set<string>>(); // questionId -> Set of Grant IDs
    const ruleUnlockMap = new Map<string, Set<string>>(); // questionId -> Set of Rule IDs
    const questionDefinitions = new Map<string, QuestionGraph>(); // questionId -> QuestionGraph

    // Helper to traverse a rule group and build lookups
    const indexRuleGroup = (group: RuleGroupGraph, grantId: string) => {
    ...
    const affectedGrants = new Set<string>();
    ...
    for (const evaluation of evaluations) {
      ...
      const extractMissing = (node: RuleGroupEvaluationResult) => {
        node.ruleResults.forEach(r => {
          if (r.state === EvaluationState.MISSING) {
            totalMissingRules++;
            ...
            if (!grantUnlockMap.has(qId)) grantUnlockMap.set(qId, new Set());
            grantUnlockMap.get(qId)!.add(evaluation.grant.grantId);

            if (!ruleUnlockMap.has(qId)) ruleUnlockMap.set(qId, new Set());
    ...
    const missingQuestions: MissingQuestionDTO[] = [];
    ...
    for (const [qId, grantSet] of grantUnlockMap.entries()) {
      ...
      const aiContext = qDef.aiContext || (qDef as any).metadata?.aiContext;
      const priority = (qDef as any).priority || 5;
      const importance = priority >= 8 ? 'HIGH' : priority >= 4 ? 'MEDIUM' : 'LOW';

      if (importance === 'HIGH') highPriorityCount++;
      ...
      missingQuestions.push({
        ...
        priority,
        aiContext,
        systemHint: (qDef as any).systemHint,
        semanticCategory: (qDef as any).semanticCategory,
        expectedAnswerType: qDef.type,
        confidenceWeight: (qDef as any).confidenceWeight,
        ...
        fieldName: (qDef as any).fieldName || qDef.fieldMapping,
        ...
        // Dependencies
        dependsOn: (qDef as any).dependsOn,
        visibilityCondition: (qDef as any).visibilityCondition,
        requiredWhen: (qDef as any).requiredWhen,
    ...
    const groupMap = new Map<string, QuestionGroupDTO>();
    let groupOrder = 1;
