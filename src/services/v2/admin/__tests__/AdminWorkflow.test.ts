...
import { ConfigurationPublishService } from '../ConfigurationPublishService';
import { RollbackService } from '../RollbackService';
import { GrantManagementService } from '../GrantManagementService';
import { IAdminRepository, IAuditLogService, IVersionRepository } from '../interfaces';

describe('Admin Configuration Workflow', () => {
  let auditMock: jest.Mocked<IAuditLogService>;
  let repoMock: jest.Mocked<IAdminRepository<any>>;
  let versionMock: jest.Mocked<IVersionRepository>;
  ...
  describe('GrantManagementService', () => {
    it('creates draft grant and logs audit', async () => {
      const service = new GrantManagementService(repoMock, auditMock);
  ...
  describe('ConfigurationPublishService', () => {
    it('aborts publish if validation fails', async () => {
      const validator = new ConfigurationValidationService();
      const service = new ConfigurationPublishService(versionMock, validator, auditMock);
  ...
  describe('RollbackService', () => {
    it('rolls back to previous version and logs audit', async () => {
      const service = new RollbackService(versionMock, auditMock);
