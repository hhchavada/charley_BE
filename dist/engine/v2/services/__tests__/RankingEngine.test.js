...
describe('RankingEngine', () => {
    const engine = new RankingEngine_1.RankingEngine();
    const createResult = (id, state, priority = 0, metadata = {}, estimatedFunding = '') => {
        return {
            grant: {
                grantId: id,
                name: `Grant ${id}`,
    ...
    it('sorts grants by global priority and recommendation score', async () => {
        const results = [
            createResult('g1', execution_1.GrantState.ELIGIBLE, 10),
            createResult('g2', execution_1.GrantState.ELIGIBLE, 50),
            createResult('g3', execution_1.GrantState.ELIGIBLE, 5)
        ];
    ...
    it('merges EDG streams and takes highest streamPriority as recommended', async () => {
        const results = [
            createResult('edg-marketing', execution_1.GrantState.ELIGIBLE, 10, { mergeGroup: 'EDG', streamPriority: 50 }),
            createResult('edg-strategy', execution_1.GrantState.ELIGIBLE, 10, { mergeGroup: 'EDG', streamPriority: 100 }),
            createResult('edg-hr', execution_1.GrantState.ELIGIBLE, 10, { mergeGroup: 'EDG', streamPriority: 20 })
        ];
        const ranking = await engine.rank(results);
    ...
    it('maps POTENTIALLY_ELIGIBLE to needsInformation bucket', async () => {
        const results = [createResult('g1', execution_1.GrantState.POTENTIALLY_ELIGIBLE, 10)];
        const ranking = await engine.rank(results);
    ...
    it('maps NOT_ELIGIBLE to hidden bucket', async () => {
        const results = [createResult('g1', execution_1.GrantState.NOT_ELIGIBLE, 10)];
        const ranking = await engine.rank(results);
    ...
    it('moves prepareNext grants to prepareNext bucket (CTC rule)', async () => {
        const results = [createResult('ctc', execution_1.GrantState.ELIGIBLE, 10, { isPrepareNext: true })];
        const ranking = await engine.rank(results);
        expect(ranking.readyNow.length).toBe(0);
        expect(ranking.prepareNext.length).toBe(1);
    });
    it('moves closed grants to windowClosed bucket', async () => {
        const results = [createResult('g1', execution_1.GrantState.ELIGIBLE, 10, { windowStatus: 'CLOSED' })];
        const ranking = await engine.rank(results);
        expect(ranking.readyNow.length).toBe(0);
    ...
    it('calculates total funding while obeying EIS stacking rule', async () => {
        const results = [
            createResult('g1', execution_1.GrantState.ELIGIBLE, 10, {}, '$30,000'),
            // EIS grant which stacks but does not increase headline total
            createResult('eis', execution_1.GrantState.ELIGIBLE, 10, { stacksWithOtherGrants: true }, '$50,000')
        ];
        const ranking = await engine.rank(results);
    ...
    it('preserves configuration badges', async () => {
        const results = [createResult('g1', execution_1.GrantState.ELIGIBLE, 10, { badges: ['Popular', 'AI Recommended'] })];
        const ranking = await engine.rank(results);
    ...
    it('calculates recommendation score based on weights', async () => {
        ...
        const result = createResult('g1', execution_1.GrantState.ELIGIBLE, 10, { aiConfidence: 8 });
