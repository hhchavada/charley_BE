...
const MissingDataResolver_1 = require("../MissingDataResolver");
const execution_1 = require("../../interfaces/execution");
describe('MissingDataResolver', () => {
    const resolver = new MissingDataResolver_1.MissingDataResolver();
    const createQuestion = (id, category, priority) => ({
        questionId: id,
        title: 'Test',
        type: 'text',
        fieldMapping: id,
    ...
    const createRule = (id, questionId) => ({
        ruleId: id,
        name: 'Rule',
        fieldPath: id,
        operator: 'equals',
    ...
    const createEvalResult = (grantId, missingRuleIds) => {
        const ruleResults = missingRuleIds.map(id => ({ ruleId: id, state: execution_1.EvaluationState.MISSING }));
        return {
            grant: { grantId },
            state: null,
            score: 0,
            ...
            rootGroupResult: {
                groupId: 'rg',
                state: execution_1.EvaluationState.MISSING,
                ruleResults,
                nestedGroupResults: []
            }
        };
