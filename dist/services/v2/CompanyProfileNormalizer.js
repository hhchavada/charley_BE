...
exports.CompanyProfileNormalizer = void 0;
class CompanyProfileNormalizer {
    normalize(payload) {
        ...
        const assignAuthoritative = (key, value) => {
        ...
        const addrParts = [];
        if (payload.block && payload.block !== 'na')
            addrParts.push(`Blk ${payload.block}`);
        if (payload.street_name && payload.street_name !== 'na')
            addrParts.push(payload.street_name);
        ...
        if (payload.building_name && payload.building_name !== 'na')
            addrParts.push(payload.building_name);
        if (payload.postal_code && payload.postal_code !== 'na')
            addrParts.push(`Singapore ${payload.postal_code}`);
...
exports.CompanyProfileNormalizer = CompanyProfileNormalizer;
